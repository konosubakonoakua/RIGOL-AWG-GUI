...
class LinePlot(pg.GraphicsLayout):
    ...
    def __init__(
        ...
        super().__init__(parent=None, border=None)
        ...
        self.timer.setInterval(int(1.0 / 60.0 * 1000.0))
    ...
    def init(self, data: Data):
        ...
        for i, c in enumerate(self.colors):
            curve = pg.PlotDataItem(
                name="Curve/{:0>2d}".format(i), pen=pg.mkPen(c, width=1)
            )
            self.plot.addItem(curve)
            self.curves.append(curve)

        self.fmt_str = self.generateFormatString()
        self.init_str = self.fmt_str.replace("{:.4f}", "///////////").replace(
            "{:+.4e}", "///////////"
        )
    ...
    def generateFormatString(self):
        ...
        for j, param in enumerate(ex_params):
            sep = (
                "&nbsp;"
                if ((i + j + 1) % 4) or (i + j == self.num_curves - 1)
                else "<br>"
            )
            string += (
                "<span style='color: white'> <b> {}".format(param)
                + " = {:+.4e}"
                + "{} </b></span>".format(sep)
            )
        string += "</span>"
    ...
    def tryAddVLine(self):
        if self.vline is None:
            self.vline = pg.InfiniteLine(
                angle=90, movable=False, pen=pg.mkPen(QColor(255, 255, 255), width=1)
            )
