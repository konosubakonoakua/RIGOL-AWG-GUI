...
class InfoButton(QPushButton):
    URL = "https://github.com/konosubakonoakua/RIGOL-AWG-GUI"
    ...
    def try_open_browser(self):
        ...
        if not success:
            ...
            print(
                "[INFO] Failed to open web browser, " "copy link to clipboard instead"
            )
...
class ControlPannel(QWidget):
    def __init__(self):
        ...
        vl = QVBoxLayout(self)
        box = QGroupBox(title="Select Device")
        box.setLayout(self.device_sel)
        vl.addWidget(box)

        hl = QHBoxLayout()
        hl.addWidget(self.down_ch1_btn)
        hl.addWidget(self.down_ch2_btn)
        box = QGroupBox(title="Download Wave")
        box.setLayout(hl)
        vl.addWidget(box)

        hl = QHBoxLayout()
        hl.addWidget(self.apply_ch1_btn)
        hl.addWidget(self.apply_ch2_btn)
        box = QGroupBox(title="Play Wave")
        box.setLayout(hl)
        vl.addWidget(box)

        hl = QHBoxLayout()
        hl.addWidget(self.info_btn)
        box = QGroupBox(title="How to Use")
        box.setLayout(hl)
        vl.addWidget(box)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        y = np.zeros_like(x)
        self.line_plot = line_plot.LinePlotWidget(line_plot.Data(x=x, y=y))
