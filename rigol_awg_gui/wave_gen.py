...
def square(
    ...
    for i in range(NUM_PTS):
        ...
        buffer[i] = square_impl(
            t, upper, lower, frequency, duty_cycle, num_cycles, delay, rest_v
        )
    return time_seq, buffer


# @njit(cache=True)
def square_impl(t, upper, lower, frequency, duty_cycle, num_cycles, delay, rest_v):
    v = 0.0
    T = 1.0 / frequency
    if num_cycles < 0:
        finish_time = np.inf
...
# @njit(cache=True)
def triangle(
    total_time, upper, lower, frequency, phase, num_cycles=-1, delay=0, rest_v=0.0
):
    buffer = np.zeros(NUM_PTS)
    time_seq = np.zeros(NUM_PTS)
    ...
    for i in range(NUM_PTS):
        ...
        buffer[i] = triangle_impl(
            t, upper, lower, frequency, phase, num_cycles, delay, rest_v
        )
    return time_seq, buffer


# @njit(cache=True)
def triangle_impl(t, upper, lower, frequency, phase, num_cycles, delay, rest_v):
    v = 0.0
    T = 1.0 / frequency
    if num_cycles < 0:
        finish_time = np.inf
...
def pulse(total_time, amps=[], widths=[], gaps=[], delay=0.0, rest_v=0.0):
...
class User(object):
    ...
    TEMPLATE = "import math\n\n{} = 10\ndef {}(t):\n    return math.sqrt(t)\n".format(
        TOTAL_TIME_NAME, IMPL_FUNC_NAME
    )
    ...
    def parse_impl(cls, impl_str: str):
        assert (
            cls.TOTAL_TIME_NAME in impl_str
        ), "`{}` not found in string, cannot determine total time.".format(
            cls.TOTAL_TIME_NAME
        )
        assert (
            cls.IMPL_FUNC_NAME in impl_str
        ), "`{}` not found in string, cannot recognize implementation.".format(
            cls.IMPL_FUNC_NAME
        )
